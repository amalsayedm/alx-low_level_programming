...
/**
 * data - Prints the data of ELF heade
 * @e_ident: the ELF data
 */
void data(unsigned char *e_ident)
{
	printf("  Data:                              ");
 ...
 * @e_ident: array containing the ELF
 */
void entry(unsigned long int e_entry, unsigned char *e_ident)
{
...
/**
 * main - Displays ELF header of an ELF file
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	{
		close(file);
		dprintf(STDERR_FILENO, "Error: Can't allocate the memmory to read file %s\n",
				argv[1]);
		exit(98);
	}
	readres = read(file, header, sizeof(Elf64_Ehdr));
	if (readres == -1)
	{
		free(header);
		close(file);
